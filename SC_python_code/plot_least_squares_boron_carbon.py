...
# it calculates residuals and least squares and plots the 
...
#find the elements of the spline energy array closest to the ams energy value:
    ...
    #L_val=10
    #D_val=12
    sum_squares=np.sum(np.square(residual))
    return closest_element,residual, sum_squares
...
#plot the data and simulation and residual with spline
def plot_sum_squares(seq):
    ### Declare some variables###
    master_path='/home/mcbride.342/galprop_sims/keith_runs_multiple/out_FITS/runs_L_D/'
    file_prefix='/nuclei_full_56_'
    ams_energy_mp, ams_ratio, ams_energy_binsize, ams_ratio_errors = read_ams_data()
    ams_energy_mp_1=np.log10(ams_energy_mp)
    energy=np.arange(0,7,0.304347391792257)
    undo_log_energy(energy)
    ...
    while D_val<21:
        ...
        while L_val<21:
            ...
            image_file=master_path+extension1+file_prefix+extension1
            image_data = fits.getdata(image_file)
            # get the ratio for that model
            He_3_He_4=np.divide(image_data[10,:,z_loc,y_loc,x_loc],image_data[11,:,z_loc,y_loc,x_loc])
            # interpolate that model
            energy_cont, He_3_He_4_spline = spline_the_ratio(energy_1,He_3_He_4)
            # calc the residuals and the sum squares
            closest_energy, residual, sum_squares = gimme_residuals(ams_energy_mp_1, ams_ratio, energy_cont, He_3_He_4_spline)
            sum_squares_array.append(sum_squares)
            L_val+=1
        sum_squares_full.append(sum_squares_array)
        print("D_Val incrementing")
        D_val+=1
    print(sum_squares_full)
    # make the L and D arrays
    ...
    cax=ax.matshow(sum_squares_full, cmap='plasma', norm=LogNorm(vmin=0.001, vmax=1))
...
#    ax.colorbar()
    ...
    #plt.tight_layout()
    plt.title("Sum of Square Residuals")
    plt.savefig("heatmap_example.png",dpi=400)

#    plt.xticks(range(20),
#          ['Category_' + str(i) + '_predicted' for i in range(0,20)], rotation = 90, 
#          fontsize = 14)
#    fnt=20
#    x1=2
#    x2=5
